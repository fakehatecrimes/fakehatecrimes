...
class Fake < ActiveRecord::Base
  ...
  validates_presence_of :media_type, message: " - a report of a fake must have a 'type', for example, 'charged with inventing crime'"
...
def date_check
  ...
  def media_check
    ...
    if self.media.size < 1 and (type.nil? || type.name.downcase != SUSPECTED.downcase)
      @errors.add( :media, "- a report of a fake must have at least one tv, radio, print or online reference unless it's merely '#{SUSPECTED}'" )
      return false
    end

    unless type.nil?
      if type.name.downcase == SUSPECTED.downcase and media.size < 1 and (date.blank? or reason.blank? or state.blank?)
        @errors.add( :reason, "- a report of a suspected fake must have a date, details and why its '#{SUSPECTED}' if there are no tv, radio, print or online references" )
        return false
      end
    end
